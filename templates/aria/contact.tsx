...
export default function AriaContact({ content }: TemplatePageProps) {
  const c = content?.contact;
  const heroEyebrow = c?.eyebrow || "Get In Touch";
  const heroTitle = c?.title || "Let's create something";
  const heroAccent = c?.accent || "beautiful together";
  const heroSubtitle = c?.tagline || "Have a question or ready to book your session? We'd love to hear from you.";
  const contactRows = c
    ? [
        { icon: Phone, label: c.phone, sub: c.phoneSub },
        { icon: Mail, label: c.email, sub: c.emailSub },
        { icon: MapPin, label: c.address, sub: c.addressSub },
      ]
    : CONTACT;
  const WHY_ICONS: Record<string, LucideIcon> = { camera: Camera, clock: Clock, calendar: Calendar, heart: Heart, shield: ShieldCheck };
  const whyItems = c?.why?.length ? c.why.map((w) => ({ icon: WHY_ICONS[w.icon] || Camera, title: w.title, sub: w.sub })) : WHY;
  const serviceNames = content?.services?.length ? content.services.map((s) => s.name) : SERVICES;
  const detailsHeading = c?.detailsHeading || "Get in touch";
  const crmNote = c?.note || "Every enquiry creates a CRM lead automatically.";
  const F = c?.form;
  const map = c?.map;
  const cta = c?.cta;
  ...
  return (
    <main className="aria font-inter bg-[var(--a-cream)] text-[var(--a-ink)]">
      ...
      <Wrap className="relative z-10 -mt-6 grid items-stretch gap-6 md:-mt-8 lg:grid-cols-[1.55fr_1fr]">
        ...
        <Reveal className="h-full">
          <div className="flex h-full flex-col rounded-2xl border border-[var(--a-line)] bg-white p-6 shadow-sm md:p-7">
            <h2 className="font-playfair text-[1.375rem] font-bold text-[var(--a-ink)]">{F?.heading || "Send us an enquiry"}</h2>
            <span className="mt-2 block h-0.5 w-10 rounded bg-[var(--a-green)]" />
            <form className="mt-5 space-y-4" onSubmit={(e) => e.preventDefault()}>
              <div className="grid gap-4 sm:grid-cols-2">
                <div>
                  <label className={label}>{F?.nameLabel || "Full name *"}</label>
                  <div className="relative">
                    <User width={16} height={16} className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-[var(--a-body)]/55" />
                    <input className={`${field} !pl-10`} placeholder={F?.namePh || "Your name"} />
                  </div>
                </div>
                <div>
                  <label className={label}>{F?.phoneLabel || "Phone *"}</label>
                  <div className="relative">
                    <Phone width={16} height={16} className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-[var(--a-body)]/55" />
                    <input className={`${field} !pl-10`} placeholder={F?.phonePh || "Your phone number"} />
                  </div>
                </div>
              </div>
              <div className="grid gap-4 sm:grid-cols-2">
                <div>
                  <label className={label}>{F?.emailLabel || "Email *"}</label>
                  <div className="relative">
                    <Mail width={16} height={16} className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-[var(--a-body)]/55" />
                    <input type="email" className={`${field} !pl-10`} placeholder={F?.emailPh || "you@email.com"} />
                  </div>
                </div>
                <div>
                  <label className={label}>{F?.serviceLabel || "Service *"}</label>
                  <div className="relative">
                    <select className={`${field} appearance-none pr-9`} defaultValue=""><option value="" disabled>{F?.servicePh || "Select a service"}</option>{serviceNames.map((s) => <option key={s}>{s}</option>)}</select>
                    <ChevronDown width={16} height={16} className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 text-[var(--a-body)]" />
                  </div>
                </div>
              </div>
              <div>
                <label className={label}>{F?.messageLabel || "Message *"}</label>
                <div className="relative">
                  <textarea rows={3} maxLength={500} value={msg} onChange={(e) => setMsg(e.target.value)} className={`${field} resize-none`} placeholder={F?.messagePh || "Tell us about your shoot, requirements, date, location or any special request..."} />
              ...
              <button type="submit" className="inline-flex items-center gap-2 rounded-lg bg-[var(--a-green-2)] px-5 py-3 text-sm font-bold text-white transition hover:-translate-y-0.5 hover:bg-[var(--a-green)]">
                <Send width={16} height={16} /> {F?.submit || "Send enquiry"}
              </button>
              <p className="flex items-center gap-1.5 text-xs text-[var(--a-body)]"><ShieldCheck width={14} height={14} className="text-[var(--a-green)]" /> {F?.privacy || "We respect your privacy. Your information is safe with us."}</p>
            </form>
          </div>
        </Reveal>
        ...
        <Reveal delay={0.08} className="h-full">
          <div className="flex h-full flex-col rounded-2xl border border-[var(--a-line)] bg-white p-6 shadow-sm md:p-7">
            ...
            <div className="mt-6 flex items-start gap-2.5 rounded-xl bg-[var(--a-green-soft)]/70 p-4">
              ...
              <p className="text-sm leading-6 text-[var(--a-ink)]">{crmNote}</p>
            </div>
          </div>
        </Reveal>
      </Wrap>
          ...
          <div className="grid gap-6 rounded-2xl border border-[var(--a-line)] bg-[#f6f7f5] p-6 lg:grid-cols-2 lg:p-8">
            ...
            <div className="relative min-h-[240px] overflow-hidden rounded-xl bg-[#dfe6e2]">
              ...
              <div className="absolute left-4 top-4 max-w-[240px] rounded-xl bg-white p-4 shadow-lg">
                <div className="text-sm font-bold text-[var(--a-ink)]">{map?.studioName || "High On Innovation Photography Studio"}</div>
                <div className="mt-1 text-xs text-[var(--a-body)]">{map?.address || "Park Street, Kolkata, West Bengal 700016"}</div>
                <a href={map?.directionsHref || "#"} className="mt-2 inline-flex items-center gap-1 text-xs font-bold text-[var(--a-green)]">{map?.directionsLabel || "Get directions"} <ArrowRight width={13} height={13} /></a>
              </div>
            </div>
          </div>
        </Reveal>
        ...
        <Reveal>
          <div className="flex flex-col items-center justify-between gap-4 rounded-2xl bg-gradient-to-r from-[var(--a-green-soft)] to-[#eef3f0] px-6 py-6 md:flex-row md:px-8">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h4 className="text-base font-bold text-[var(--a-ink)]">{cta?.heading || "Ready to capture your story?"}</h4>
                <p className="text-sm text-[var(--a-body)]">{cta?.subtitle || "Book your session today and let's create memories that last forever."}</p>
              </div>
            </div>
      ...
      <CommonSections content={content} page="contact" />
    </main>
  );
}
